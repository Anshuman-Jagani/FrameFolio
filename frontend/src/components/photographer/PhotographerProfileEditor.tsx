...
export default function PhotographerProfileEditor({
  ...
  if (!profile) {
    return (
      <div className="rounded-3xl border border-taupe-100 bg-cream-50 p-6">
        <div className="text-sm font-semibold text-charcoal-700">Profile editor</div>
        <div className="text-sm text-taupe-700 mt-2">Load your profile to edit details.</div>
      </div>
    )
  }

  return (
    <div className="rounded-3xl border border-taupe-100 bg-cream-50 p-6 sm:p-8 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-lg font-serif font-semibold text-charcoal-700">Profile edit</div>
          <div className="text-sm text-taupe-700 mt-1">
      ...
      <div className="mt-6 grid gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-medium text-taupe-700" htmlFor="rate">
              Hourly rate (per day)
            </label>
            <input
              id="rate"
          ...
          <div>
            <label className="text-sm font-medium text-taupe-700" htmlFor="location">
              Location
            </label>
            <input
              id="location"
        ...
        <div>
          <label className="text-sm font-medium text-taupe-700" htmlFor="tags">
        ...
        <div>
          <label className="text-sm font-medium text-taupe-700" htmlFor="ig">
            Instagram URL
          </label>
          <input
            id="ig"
        ...
        <div className="flex gap-3 flex-wrap">
          <button
            type="button"
            className="rounded-2xl bg-burgundy-500 text-cream-50 px-5 py-3 text-sm font-medium hover:bg-burgundy-600 transition-colors disabled:opacity-50"
            disabled={loading}
      ...
      <div className="mt-8">
        ...
        {portfolioItems.length === 0 ? (
          <div className="mt-3 rounded-2xl border border-taupe-100 bg-cream-50 p-5 text-sm text-taupe-700">
          ...
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
            {portfolioItems.map((it) => (
              <div
                key={it.id}
                className="rounded-2xl border border-taupe-100 bg-cream-50 overflow-hidden"
              >
                {it.mediaType === 'image' ? (
                  <img src={it.mediaUrl} alt="Portfolio item" className="w-full h-28 object-cover" loading="lazy" />
                ) : (
                  <div className="w-full h-28 flex items-center justify-center text-sm text-taupe-700">
                    Video
                  </div>
                )}
                <div className="p-2 flex items-center justify-between gap-2">
                  <div className="text-xs text-taupe-700 truncate">{it.category ?? '—'}</div>
                  <button
                    type="button"
        ...
        <div className="mt-6 rounded-2xl border border-taupe-100 bg-cream-100/60 p-4">
          <div className="text-sm font-semibold text-charcoal-700">Add a new item</div>
          <div className="mt-3 grid grid-cols-1 gap-3">
            <div>
              <label className="text-xs font-medium text-taupe-700" htmlFor="mediaUrl">
              ...
              <input
                ...
                placeholder="https://…"
                className="mt-2 w-full rounded-2xl border border-taupe-100 bg-cream-50 px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-burgundy-500/30 text-charcoal-700 placeholder:text-taupe-500"
              />
            </div>
            <div>
              <label className="text-xs font-medium text-taupe-700" htmlFor="mediaType">
                Media type
              </label>
              <select
                id="mediaType"
                ...
                className="mt-2 w-full rounded-2xl border border-taupe-100 bg-cream-50 px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-burgundy-500/30 text-charcoal-700"
              >
        ...
        <div className="mt-4 text-xs text-taupe-600">
