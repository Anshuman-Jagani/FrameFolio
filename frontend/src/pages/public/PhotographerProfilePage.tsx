...
export default function PhotographerProfilePage() {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="rounded-3xl border border-parchment-200 bg-parchment-200 p-8">
          ...
          <div className="text-sm text-olive-500 mt-2">Try browsing the list.</div>
          <button
            type="button"
            className="mt-6 rounded-2xl bg-pine-500 text-parchment-50 px-5 py-3 text-sm font-medium hover:bg-pine-600 transition-colors"
  ...
  const availabilityTone: Record<Photographer['availability'], string> = {
    Available: 'bg-pine-50 text-pine-500 border-pine-100',
    Limited: 'bg-parchment-200 text-charcoal-700 border-parchment-200',
    Unavailable: 'bg-charcoal-700/10 text-charcoal-700 border-parchment-200',
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <section className="rounded-3xl border border-parchment-200 bg-parchment-200/80 backdrop-blur p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="flex-1">
            <div className="flex items-start gap-6">
              <img
                ...
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl border border-parchment-200 object-cover"
              />
              <div>
                ...
                <div className="mt-1 text-olive-500">
                ...
                <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-parchment-200 bg-parchment-100/80 px-3 py-1 text-sm">
                  <span className="text-olive-500">Availability:</span>
                  <span
                    className={[
          ...
          <div className="w-full lg:w-[340px]">
            <div className="rounded-3xl border border-parchment-200 bg-parchment-100 p-5 shadow-sm">
              <div className="text-sm text-olive-500">From</div>
              <div className="mt-1 flex items-baseline gap-2">
                <div className="text-4xl font-serif font-semibold text-charcoal-700">
                  {formatAed(photographer.price)}
                </div>
                <div className="text-sm text-olive-500">per day</div>
              </div>
              <div className="mt-5">
                <label className="text-xs font-medium text-olive-500" htmlFor="shoot-date">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-2xl border border-parchment-200 bg-parchment-100 px-4 py-3 text-sm text-charcoal-700"
                />
              </div>
              <button
                type="button"
                className="mt-4 w-full rounded-2xl bg-pine-500 text-parchment-50 px-4 py-3 text-sm font-medium hover:bg-pine-600 transition-colors"
                onClick={() =>
              ...
              {user?.role === 'client' ? (
                <Link
                  to="/dashboard/client"
                  className="mt-3 block w-full rounded-2xl border border-parchment-200 text-center text-sm font-medium text-charcoal-700 py-3 hover:bg-parchment-200 transition-colors"
                >
                  Message from dashboard
                </Link>
              ) : (
                <p className="mt-3 text-xs text-copper-600 text-center">
      ...
      <section className="mt-10">
        ...
        <p className="text-sm text-olive-500 mt-1">Indicative bundles in AED — confirm details in your booking thread.</p>
        <div className="mt-5 grid md:grid-cols-3 gap-4">
          {packages.map((pkg) => (
            <div
              key={pkg.name}
              className="rounded-3xl border border-parchment-200 bg-parchment-100 p-5 flex flex-col"
            >
              ...
              <p className="mt-2 text-sm text-olive-500 flex-1">{pkg.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="text-sm text-olive-500">Recent work from this creative.</div>
        </div>
        {photographer.portfolio.length === 0 ? (
          <div className="mt-5 rounded-2xl border border-parchment-200 bg-parchment-200 p-8 text-center text-olive-500 text-sm">
          ...
          <div className="mt-5 grid grid-cols-2 md:grid-cols-3 gap-4">
            {photographer.portfolio.map((src, idx) => (
              <div
                key={src + idx}
                className="rounded-3xl overflow-hidden border border-parchment-200 bg-parchment-200"
              >
                <img
                  src={src}
      ...
      <section className="mt-10 rounded-3xl border border-parchment-200 bg-parchment-200/80 p-6 shadow-sm">
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 rounded-2xl border border-parchment-200 bg-parchment-100 p-8 text-center text-sm text-olive-500">
